...
Preserve only a one-way `EvidenceExporter` seam after Evidence Bundle finalization. A future OTLP/Langfuse exporter must be asynchronous, idempotent, redacted, and optional; exporter failure cannot alter the Incident Run, bundle, verdict, or demo state.
